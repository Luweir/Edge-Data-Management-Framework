...
class AlphaRelativeDensityRatioEstimator(Estimator):
    ...
    to be learned from the insurance samples. The parameters theta in the model
    ...
    def H_hat(alpha=0.0, KernelMatrix_refSamples=None, KernelMatrix_testSamples=None):
        ...
        """
        N_ref = KernelMatrix_refSamples.shape[1]   # 读取矩阵的列数
...
class RULSIF(Estimator):
    ...
    @staticmethod
    def getMedianDistanceBetweenSamples(sampleSet=None):
        """
        ...
        labels.（Jaakkola设置高斯径向基函数核宽度参数的启发式方法是选取具有不同标签的
        点之间欧氏距离分布的分位数(通常是中值)。）
        Reference:
        ...
        """
        numrows = sampleSet.shape[0]  # 获取矩阵的行数
        samples = sampleSet

        G = numpy.sum((samples * samples), axis=1)   # 当axis为1时,是压缩列,即将每一行的元素相加,将矩阵压缩为一列
        Q = numpy.tile(G[:, None], (1, numrows))    # 扩展矩阵
        R = numpy.tile(G, (numrows, 1))

        distances = Q + R - 2 * numpy.dot(samples, samples.T)  # numpy.dot  矩阵点乘
    ...
    def computeGaussianWidthCandidates(self, referenceSamples=None, testSamples=None):
        ...
        """
        # numpy.c_[a,b] 将矩阵直接合并,即(a|b)
        allSamples = numpy.c_[referenceSamples, testSamples]
        medianDistance = self.getMedianDistanceBetweenSamples(allSamples.T)

        # xmy-start
        GaussianWidthCandidates = medianDistance * numpy.array([0.6, 0.8, 1, 1.2, 1.4])
        # GaussianWidthCandidates = medianDistance * numpy.array([0.8, 0.9, 1, 1.1, 1.2])
        print("GaussianWidthCandidates....: ", GaussianWidthCandidates)
        return GaussianWidthCandidates
        # xmy-end
    ...
    def generateRegularizationParams(self):
        ...
        """
        # xmy-start
        RegularizationParamsCandidates = 10.0 ** numpy.array([-3, -2, -1, 0, 1])
        print("RegularizationParamsCandidates....: ", RegularizationParamsCandidates)
        return RegularizationParamsCandidates
        # xmy-end
    ...
    def generateAllGaussianCenters(self, referenceSamples=None):
        ...
        """
        self.kernelBasis = referenceSamples.shape[1]      # 获取矩阵的列的数（维度）
        return referenceSamples[:, numpy.r_[0:self.kernelBasis]]    # numpy.r_[a,b]  按列连接a,b两个矩阵
    ...
    def computeModelParameters(self, referenceSamples=None, testSamples=None, gaussianCenters=None):
        ...
        print("sigmaWidths...: ", sigmaWidths)
        lambdaCandidates = self.generateRegularizationParams()
        print("lambdaCandidates...: ", lambdaCandidates)
        ...
        referenceSamplesCVIdxs = numpy.random.permutation(refCols)  # numpy.random.permutation 随机排列一个序列，返回排列后的序列
        referenceSamplesCVSplit = numpy.floor(numpy.r_[0:refCols] * self.crossFolds / refCols)  # numpy.floor 取下限值
        ...
        for sigmaIdx in numpy.r_[0:numpy.size(sigmaWidths)]:
            ...
            # print("K_ref(xmy): ", K_ref)
            K_test = GaussianKernel(sigma).apply(testSamples, gaussianCenters).T
            # print("K_test(xmy): ", K_test)
            # Initialize a new result matrix for the current sigma candidate
            foldResult = numpy.zeros((self.crossFolds, numpy.size(lambdaCandidates)))  # numpy.zeros 返回来一个给定形状和类型的用0填充的数组
            # print("foldResult(xmy) :", foldResult)
            # print("self.crossFolds(xmy): ", self.crossFolds)
            # xmy-start
            for foldIdx in numpy.r_[1:self.crossFolds+1]:
            # xmy-end
            # for foldIdx in numpy.r_[0:self.crossFolds]:
                # print("foldIdx(xmy): ", foldIdx)
                K_ref_trainingSet = K_ref[:, referenceSamplesCVIdxs[referenceSamplesCVSplit != foldIdx]]
                # print("K_ref_trainingSet: ", K_ref_trainingSet)
                K_test_trainingSet = K_test[:, testSamplesCVIdxs[testSamplesCVSplit != foldIdx]]
                # print("K_test_trainingSet: ", K_test_trainingSet)
                ...
                for lambdaIdx in numpy.r_[0:numpy.size(lambdaCandidates)]:
                    ...
                    # xmy-start
                    foldResult[foldIdx-1, lambdaIdx] = J
                    # xmy-end
        ...
        print("crossValidationMinScores...: ", crossValidationMinScores)
        crossValidationMinIdxForLambda = crossValidationScores.argmin(1)
        print("crossValidationMinIdxForLambda...: ", crossValidationMinIdxForLambda)
        crossValidationMinIdxForSigma = crossValidationMinScores.argmin()
        print("crossValidationMinIdxForSigma...: ", crossValidationMinIdxForSigma)
        ...
        print("optimalSigma : ", optimalSigma)
        print("optimalLambda : ", optimalLambda)
    ...
    def train(self, referenceSamples=None, testSamples=None):
        ...
        print("optimalSigma is : ", optimalSigma)
        self.lambdaRegularizer = optimalLambda
        print("optimalLambda is : ", optimalLambda)
