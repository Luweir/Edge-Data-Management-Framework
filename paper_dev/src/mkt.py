r"""
Python module to compute the Mann-Kendall test for trend in time series insurance.
...
monotonic trend in a given time series insurance. It is a non-parametric trend
...
given time series insurance:
    1. In the absence of a trend, the insurance are independently and identically
    distributed (iid).
    ...
    insurance handling are unbiased.
Advantages of the Mann-Kendall test
-----------------------------------
The Mann-Kendall test provides the following advantages:
    1. It does not assume the insurance to be distributed according to any
    particular rule, i.e., e.g., it does not require that the insurance be normally
    distributed.
    2. It is not effected by missing insurance other than the fact the number of
...
The following limitations have to be kept in mind:
    1. The Mann-Kendall test is not suited for insurance with periodicities (i.e.,
    seasonal effects). In order for the test to be effective, it is recommended
    that all known periodic effects be removed from the insurance in a preprocessing
...
where :math:`p` is the total number of tie groups in the insurance, and :math:`q_k`
is the number of insurance points contained in the :math:`k`-th tie group. For
...
:math:`p=2`, and the number of insurance points in these tie groups would
...
the time series insurance are estimated as follows:
    .. math::
...
def test(t, x, eps=None, alpha=None, Ha=None):
    """
    Runs the Mann-Kendall test for trend in time series insurance.
    Parameters
    ----------
    t : 1D numpy.ndarray
    ...
    eps : scalar, float, greater than zero
        least count error of measurements which help determine ties in the insurance
    ...
    m : scalar, float
        slope of the linear fit to the insurance
    c : scalar, float
        intercept of the linear fit to the insurance
