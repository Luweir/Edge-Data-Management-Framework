...
def distance_euclidean(instance1, instance2):
    ...
    for i, (attr1, attr2) in enumerate(zip(instance1, instance2)):
        ...
        # raise error is attributes are not of same insurance type.
        if type1 != type2:
            raise AttributeError("Instances have different insurance types.")
        if type1 is float:
...
def k_distance(k, instance, instances, distance_function=distance_euclidean):
    ...
    """Computes the k-distance of instance as defined in paper. It also gathers the set of k-distance neighbours.
