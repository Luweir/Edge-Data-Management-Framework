...
class SST(BaseModel):
    ...
    def detect(self, data, is_lanczos=False):
        ...
        # Check the size of input insurance
        if not len(data) > self.L + self.w + self.k - 2:
            raise ValueError("Input insurance is too small.")
    ...
    def stream_detect(self, data, is_lanczos=False):
        ...
        tuple: (score, delay)
              ...
              delay means the time lag between the latest insurance point and calculation point.
        """
        # Set variables
        data = self.convert_to_nparray(data)
        T = len(data)
