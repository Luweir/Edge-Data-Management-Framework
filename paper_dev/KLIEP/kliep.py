...
class DensityRatioEstimator:
    ...
    def __init__(self, max_iter=5000, num_params=[0.1, 0.2], epsilon=1e-4, cv=3, sigmas=[.01, .1, .25, .5, .75, 1],
    ...
    def fit(self, X_train, X_test, alpha_0=None):
        ...
        if len(self.sigmas) * len(self.num_params) > 1:
            ...
            sorted_scores = sorted([x for x in j_scores.iteritems() if np.isfinite(x[1])], key=lambda x: x[1],
                                   reverse=True)
